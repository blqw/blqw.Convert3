...
namespace blqw.Converts
{
    ...
    public class CDataTable : BaseTypeConvertor<DataTable>
    {
        ...
        /// <returns> </returns>
        protected override DataTable ChangeTypeImpl(IConvertContext context, object input, Type outputType, out bool success)
        {
            success = true;
            if (input is DataView view)
            {
                return view.ToTable();
            }
        ...
        /// <param name="outputType"> 换转后的类型 </param>
        /// <param name="success"> 是否成功 </param>
        protected override DataTable ChangeType(IConvertContext context, string input, Type outputType, out bool success)
        {
            input = input.Trim();
            if ((input[0] == '{') && (input[input.Length - 1] == '}') 
                && context.Services.GetService(typeof(IJsonService)) is IJsonService json)
            {
                try
                {
                    var result = json.ToJsonObject(outputType, input);
                    success = true;
        ...
        protected struct DataTableBuilder : IBuilder<DataTable, object>
        {
            ...
            public DataTableBuilder(IConvertContext context)
            {
                _context = context;
                _columns = null;
                Instance = null;
