...
namespace blqw.Converts
{
    ...
    public abstract class BaseTypeConvertor<T> : BaseConvertor<T>
    {
        ...
        protected sealed override T ChangeType(IConvertContext context, object input, Type outputType, out bool success)
        {
        ...
        /// <param name="success"> 是否成功 </param>
        /// <returns> </returns>
        protected abstract T ChangeTypeImpl(IConvertContext context, object input, Type outputType, out bool success);
            ...
            where TOutput : T
        {
            ...
            /// <param name="outputType"> 换转后的类型 </param>
            /// <param name="success"> 是否成功 </param>
            TOutput IConvertor<TOutput>.ChangeType(IConvertContext context, string input, Type outputType, out bool success)
            ...
            /// <param name="outputType"> 换转后的类型 </param>
            /// <param name="success"> 是否成功 </param>
            TOutput IConvertor<TOutput>.ChangeType(IConvertContext context, object input, Type outputType, out bool success)
                => (TOutput) context.Get<T>().ChangeType(context, input, outputType, out success);
